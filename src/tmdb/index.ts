...
const proxyTmdb = async (c: import('hono').Context) => {
  ...
  // Normalize query param order for a stable cache key
  ...
  // Map /tmdb/<suffix> -> https://api.themoviedb.org/<suffix>
  const suffix = incomingUrl.pathname.replace(/^\/tmdb\/?/, '')
  const base = 'https://api.themoviedb.org'
  const target = new URL(base + (suffix ? `/${suffix}` : '') + normalizedSearch)

  // Forward headers (strip hop-by-hop/personalization) and inject our TMDB API key
  ...
  // Force JSON accept per example
  headers.set('accept', 'application/json')
  // Hide API key by injecting server-side Authorization
  headers.set(
    'authorization',
    'Bearer TMDB_KEY_REDACTED'
  )
...
// Handle both /tmdb and /tmdb/* when mounted
tmdb.all('/', proxyTmdb)
tmdb.all('/*', proxyTmdb)
